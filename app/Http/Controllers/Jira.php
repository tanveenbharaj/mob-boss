...
class Jira
{
    ...
    /**
     * addCommentToTask function to add comment to Jira Task
     *
     * @param string $issue
     * @param array $data
     * @return mixed
     */
    public static function addCommentToTask( $id, $data )
    {
        $data   = json_encode( array( 'body' => $data ) );
        //dd($data);
        $result = self::requestToAddComment( $id, $data,1,0 );
    ...
    private static function requestToAddComment( $id, $data, $is_post = 0, $is_put = 0 )
    {

        $ch = curl_init();
        curl_setopt_array( $ch, array(
            CURLOPT_URL            => config( 'jira.url' ) . '/rest/api/2/issue/'.$id.'/comment',
            CURLOPT_USERPWD        => config( 'jira.username' ) . ':' . config( 'jira.password' ),
            CURLOPT_POSTFIELDS     => $data,
            CURLOPT_HTTPHEADER     => array( 'Content-type: application/json' ),
            CURLOPT_RETURNTRANSFER => 1,
